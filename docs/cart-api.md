...
|---|---|---|
| کاربر لاگین‌شده | کوکی `session` (HttpOnly) | مرورگر، خودکار |
...
کوکی `csrf_token` بعد از لاگین و `/auth/otp/verify` **rotate** می‌شود (و پاسخ بدنه‌شان توکن جدید را برنمی‌گرداند). اگر توکن را در state/axios-default نگه داری، بعد از هرکدام از این‌ها stale می‌شود → `403`. همیشه لحظه‌ی درخواست از `document.cookie` بخوان.
...
### تله ۴ — نشست نامعتبر کاربر را بی‌صدا مهمان می‌کند
`OptionalSessionAuthGuard` اگر کوکی نشست نامعتبر/منقضی/باطل‌شده باشد خطا نمی‌دهد و درخواست را **مهمان** فرض می‌کند. یعنی کاربری که نشستش باطل شده:
- اگر `X-Cart-Token` بفرستد → آیتم در سبد مهمان می‌رود؛
- اگر نفرستد → روی mutation خطای `400` می‌گیرد.

راه‌حل: قبل از عملیات سبد با `GET /auth/me` مطمئن شو نشست معتبر است؛ برای کاربر لاگین‌شده یک fallback توکن مهمان داشته باش، یا `400` را «نشست منقضی» تفسیر کن و کاربر را به لاگین ببر.

### تله ۵ — چند تب، یک کوکی CSRF
کوکی `csrf_token` بین تب‌ها مشترک است. اگر یک تب لاگین کند، تب دیگر با توکن cache‌شده ۴۰۳ می‌گیرد. فقط با «خواندن تازه از کوکی» حل می‌شود.

### تله ۶ — چرخش CSRF بعد از ورود
بعد از هر login/otpVerify، کوکی `csrf_token` عوض می‌شود. اگر توکن را cache کرده باشی و همان را بفرستی، `403` می‌گیری. همیشه قبل از mutation، توکن را تازه از کوکی بخوان.
